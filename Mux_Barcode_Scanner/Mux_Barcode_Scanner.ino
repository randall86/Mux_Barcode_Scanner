...
#define GET_MUX_BIT(A, B) ((A >> (B & MUX_MASK)) & 1)
...
void selectBScanner(byte index)
{
  ...
  digitalWrite(MUX_A0, GET_MUX_BIT(index, 0));
  digitalWrite(MUX_A1, GET_MUX_BIT(index, 1));
  digitalWrite(MUX_A2, GET_MUX_BIT(index, 2));
...
void scanBScanner(byte index)
{
  ...
  g_ioExpandr.digitalWrite0(index, LOW);
  ...
  g_ioExpandr.digitalWrite0(index, HIGH);
}
...
void debounceSWRoutine()
{
  ...
  if(debounceSW(&sw_state))
  {
    if(sw_state)
    {
      g_isFixtureDetected = 1;
    }
    else
    {
      g_isFixtureDetected = 0;
    }
  }
}
...
void loop()
{
  ...
  {
    //turn on the fixture detected LED
  ...
  {
    //turn off the LED when fixture is removed
    digitalWrite(FIXTURE_LED, LOW);
  }
}
